  ...
  return (
    <Card className="min-h-[260px]">
      <div className="flex items-center justify-between gap-3">
        ...
        <span className="rounded-md bg-slate-100 px-2 py-1 text-xs font-medium text-slate-600">{data.length} trạng thái</span>
      </div>
      {data.length === 0 ? <p className="mt-8 text-sm text-slate-500">Chưa có dữ liệu pipeline.</p> : null}
      <ul className="mt-4 space-y-3 text-sm">
        {data.map((item) => (
          <li className="space-y-1.5" key={item.status ?? 'unknown'}>
            <div className="flex items-center justify-between gap-3">
              <span className="truncate text-slate-700">{item.status ?? 'Chưa phân loại'}</span>
